...
data.products.forEach((product: Product) => {
  ...
  if (!vatTotalsMap[vatRate]) {
    vatTotalsMap[vatRate] = { nettAmount: 0, taxAmount: 0 };
  }
  vatTotalsMap[vatRate].nettAmount += productTotalExcl; // Sum the net amounts for the VAT rate
  vatTotalsMap[vatRate].taxAmount += productTaxAmount; // Sum the tax amounts for the VAT rate
});

// Construct the receipt XML
...
// Append products to the ITEMS section
data.products.forEach((product: Product, index: number) => {
  receiptData
    ...
    .up()
    .ele('TAXCODE', product.tax_id)
    .up()
    .ele('AMT', product.selling_price)
    .up()
    .up(); // Ensure each ITEM is closed properly
});

// Close the ITEMS section
receiptData.up(); // Moves up to close the ITEMS block

// Now append the TOTALS section outside of ITEMS
receiptData
  .ele('TOTALS')
...
// Append VATTOTALS section
const vatTotals = receiptData.ele('VATTOTALS');

Object.keys(vatTotalsMap).forEach((vatRate) => {
  const totals = vatTotalsMap[vatRate];
  vatTotals
    .ele('VATRATE', vatRate)
    .up()
    .ele('NETTAMOUNT', totals.nettAmount.toFixed(2))
    .up()
    .ele('TAXAMOUNT', totals.taxAmount.toFixed(2))
    .up();
});

// Finalize and return the receipt XML string
// const xmlString = receiptData.end({ pretty: true });
