  ...
  return new Promise((resolve, reject) => {
    db.all(
      ...
      SELECT s.id AS sale_id, s.date, s.invoice_description AS invoice_description,
             ...
             c.name AS customer_name, c.address, c.phone, c.tin, c.email, c.VRN,
      ...
      LEFT JOIN Tax t ON p.tax_id = t.id
       WHERE p.itemType = 'Service'  -- Only include products marked as services
      ORDER BY s.id DESC
    `,
      [],
      (err, rows) => {
  ...
  return new Promise((resolve, reject) => {
    db.all(
      ...
      SELECT s.id AS sale_id, s.date, s.invoice_description AS invoice_description,
             s.total_amount, s.invoice_number, s.status,
             c.name AS customer_name, c.address, c.phone, c.tin, c.email, c.VRN,
             si.product_id, p.name AS product_name, p.itemCode, p.description AS product_description, si.quantity AS sold_quantity,
             si.price AS selling_price, pi.price AS buying_price,
             (CASE WHEN p.itemType != 'Service' THEN (si.quantity * si.price) - (pi.quantity * pi.price) ELSE NULL END) AS profit,
             t.codeType AS tax_codeType, t.codeValue AS tax_codeValue, t.id AS tax_id,
             pf.profoma_number AS profoma_number
      FROM Sales s
      JOIN SalesItems si ON s.id = si.sale_id
      JOIN Products p ON si.product_id = p.id
      LEFT JOIN PurchaseItems pi ON p.id = pi.product_id AND p.itemType != 'Service'
      JOIN Customers c ON s.customer_id = c.id
      LEFT JOIN Tax t ON p.tax_id = t.id
      LEFT JOIN profoma pf ON s.proforma_id = pf.id
      ORDER BY s.id DESC
      `,
      [],
      (err, rows) => {
        if (err) {
          ...
          console.log('Sales details fetched:', rows); // Log fetched sales details
          const salesDetails = rows.reduce(
            (acc: { [key: number]: SaleDetail }, row) => {
              const {
                sale_id,
              ...
              acc[sale_id].products.push({
                ...
                selling_price,
                tax_id,
                buying_price: buying_price !== null ? buying_price : undefined, // Only non-serviced products have buying price
                tax_codeType,
                tax_codeValue,
                profit: profit !== null ? profit : undefined, // Only non-serviced products have profit
              });
...
ipcMain.handle('fetch-products_sales', async (event): Promise<any[]> => {
  return new Promise((resolve, reject) => {
    db.all(
      `
      SELECT p.id, p.name, p.description, p.price, p.itemCode,p.itemType as itemtype,
