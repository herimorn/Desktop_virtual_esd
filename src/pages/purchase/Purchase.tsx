...
const exportToCSV = (purchases: Purchase[]) => {
  ...
  const handleDateFilterChange = (event: ChangeEvent<HTMLInputElement>) => {
    setDateFilter(event.target.value); // Store as string directly
  };
  ...
  const filterPurchases = () => {
    return purchases.filter((purchase) => {
      ...
      const dateMatches =
        dateFilter && String(dateFilter).trim() !== "" // Ensure dateFilter is a string before trimming
          ? new Date(dateFilter).toLocaleDateString("en-GB") ===
            purchaseDate.toLocaleDateString("en-GB")
          : true;
  
      const monthMatches = monthFilter
        ? purchaseDate.getMonth() + 1 === parseInt(monthFilter)
        : true;
  
      const yearMatches = yearFilter
        ? purchaseDate.getFullYear() === parseInt(yearFilter)
        : true;
...
const renderTableRows = () => {
  ...
  <Button className='btn btn-secondary btn-sm rounded-pill font' onClick={handleAddPurchase} style={{ width: 200 }}>
...
<h3 style={{position:'relative',top:-110}} >All Purchase</h3>
