...
import { ArrowLeft, Upload,Table } from 'react-bootstrap-icons';
import {
  Cursor,
  Crop,
  ArrowsMove,
...
// Add type for preview state
interface PreviewState {
  type: 'header' | 'footer';
  image: string;
}

export const ProfomaCustomazation: React.FC = () => {
  ...
  const [profoma, setProfoma] = useState<Profoma | null>(null);
  ...
  const [profomaDetail,setProfomaDetail] = useState([]);
  ...
  const [currentPreview, setCurrentPreview] = useState<PreviewState | null>(null);
  console.log("the profoma is",profoma)
...
const [customStyles, setCustomStyles] = useState({
  ...
  }, [profoma_id, profoma?.profoma_number]);

//   // Fetch custom styles and positions from the backend when the component mounts
//   window.electron.fetchStyles(profoma.profoma_number).then(data => {
...
navigate(`/profoma/${profoma_id}`);

};
const handleDragStop = (e: any, data: { x: number; y: number }, sectionId: string) => {
...
//then please save the data to the backend database
  ...
  const  fetchCustomizations = async () =>{
    ...
    const  response = await window.electron.fetchCustomizationData(profoma.profoma_id,profoma.profoma_number);
    ...
    const fetchProfomaDetails = async () => {
      try {
        ...
        const profomaDetail = response.find(
          (profoma) => profoma.profoma_id.toString() === profoma_id
        );
        setProfoma(profomaDetail || null);
        if (profomaDetail) {
          await window.electron.fetchBarcodeDetails(
            profomaDetail.profoma_id,
            profomaDetail.profoma_number
          );
        }
      } catch (error) {
        console.error('Error fetching Profoma details:', error);
  ...
  const calculateTotal = () => {
    if (!profoma) return 0;
    return profoma.products.reduce((sum, product) => sum + product.selling_price * product.sold_quantity, 0);
  };
  ...
  const calculateTaxAmount = (exclusiveAmount: number) => {
    if (!profoma || !profoma.tax_codeValue) return 0;
    const taxPercentage = parseFloat(profoma.tax_codeValue) / 100;
  ...
  const handleSave = () => {
    console.log(profoma.profoma_id,profoma.profoma_number)
    setShowPopup(false);
    selectedSections.forEach(sectionId => {
      window.electron.saveStyles(sectionId, customStyles, positions[sectionId]);
    });
    ...
    input.onchange = async (e) => {
      ...
      if (target.files && target.files[0]) {
        try {
          ...
          setCurrentPreview({ type: 'header', image: base64Image });
  ...
  const handleSavePreview = async () => {
    ...
    try {
      const section = currentPreview.type === 'header'
      ...
      if (section) {
        ...
        img.src = currentPreview.image;
        img.style.cssText = currentPreview.type === 'header'
          ? `
            width: 102%;
            height: 102%;
            object-fit: cover;
        ...
        // Save to database
        await window.electron.updatePraformaCustomization({
          [`${currentPreview.type}_images`]: currentPreview.image,
          profoma_number: profoma.profoma_number
        });
    ...
    <Container fluid className="customization-container font"style={{fontFamily:'CustomFont'}}>
      <Row className="mb-4 align-items-center">
        ...
        <Button onClick={handleBack} className="mb-3" style={{ position:'relative',width:'10%',left:-600,marginLeft:'95%',backgroundColor:'#6c757d'}}>
          ...
          <h1 className="fw-bold font">Customize Profoma</h1>
        </Col>
  ...
  <Draggable position={positions.billTo || { x: 0, y: 0 }}
    ...
    <div id="billTo" onClick={() => handleSectionClick('billTo')}
     style={{ ...styles.billTo, ...applyCustomStyles('billTo') }}>
      {/* Removed the two <br/> tags from here */}
      <strong>COMPANY NAME:</strong>{'  '}{profoma.customer_name}
      <br />
      <strong>TIN:</strong>{'   '}{profoma.customer_tin}
      <br />
      <strong>ADDRESS:</strong>{'   '}{profoma.customer_address}
      <br />
      <strong>MOB:</strong>{'   '}{profoma.customer_phone}
      <br />
      <strong>EMAIL:</strong>{'   '}{profoma.customer_email}
      <br />
    </div>
  </Draggable>
  <Draggable position={positions.invoiceInfo || { x: 0, y: 0 }}
    ...
    <div id="invoiceInfo" onClick={() => handleSectionClick('invoiceInfo')}
     style={{ ...styles.invoiceInfo, ...applyCustomStyles('invoiceInfo') }}>
      ...
      {profoma.profoma_number}
      <br />
      <strong>Date: </strong>
      {new Date(profoma.date).toLocaleDateString()}
      <br />
    </div>
  </Draggable>
</div>
  ...
  {new Intl.NumberFormat('en-US', {
    style: 'currency', currency: 'TZS', minimumFractionDigits: 0
      ...
      maximumFractionDigits: 0}).format(grandTotal)}</strong></td>
        ...
        <div>
        <div dangerouslySetInnerHTML={{ __html: profoma.invoice_description }} />
...
>
  ...
  <div>
    ...
    onClick={handleSave}
    style={{ position: 'absolute', right: 0, top: '70%', width: 300 }}
>
    Save Customization
</button>
...
export default ProfomaCustomazation;
